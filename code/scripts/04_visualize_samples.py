...
def main() -> None:
    ...
    parser.add_argument(
        "--subset-sheets",
        action="store_true",
        help="Also write per-subset QA contact sheets (drone_positive, distractor_only, mixed)",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    split_stem = cfg.split.replace("/", "_")
    stem = f"{split_stem}_contact_sheet_qa" if use_full_qa else f"{split_stem}_contact_sheet"
    out = args.output or (sheets_dir / f"{stem}.png")
    crops_out = sheets_dir / (
        f"{split_stem}_contact_sheet_crops_qa.png"
        if use_full_qa
        else f"{split_stem}_contact_sheet_crops.png"
    )
    ...
    if use_full_qa:
        html_path = build_html_audit_report(
            cfg.metadata_csv,
            cfg.images_dir,
            reports_dir / f"{split_stem}_audit.html",
    ...
    if args.subset_sheets and use_full_qa:
        subset_short_names = {
            "synthetic_drone_positive": "drone_positive",
            "synthetic_distractor_only": "distractor_only",
            "synthetic_drone_plus_distractor": "drone_plus_distractor",
        }
        for subset_name, short in subset_short_names.items():
            sub_out = sheets_dir / f"{split_stem}_{short}_qa.png"
            build_contact_sheet(
                cfg.metadata_csv,
                cfg.images_dir,
                sub_out,
            ...
            print(f"Saved subset contact sheet: {sub_out}")
